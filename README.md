...
不能与正式 `corpus` 口径比较。报告schema升级为2，旧版固定理想排名产生的nDCG应作废，不直接比较。
...
触发后直接列出分歧和对应引用，不调用答案生成或引用核对模型，也不继续生成其余回答。没有匹配到登记规则不代表不存在冲突。
这是针对已知教材问题的保护，不是通用语义冲突检测；详见 [证据复核与处理](docs/database-evidence-review.md)。
