...
BM25 Recall@5 由此从 0.400 提升到 0.700，权重随之从 0.2 上调到 0.3。重排后的
Recall@5 在权重 0.1 到 0.6 之间稳定为 1.000，0.8 时掉到 0.900，区间内 MRR 的差异
不足一道题，因此取区间中部留出余量，而不是取端点。

标注集只有 10 题，以上数值都是暂定的；分词或标注集变化后都应重新扫描。
