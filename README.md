...
检索层在 65 题标注集上的结果（不调用 LLM；Hybrid + Reranker 一行跨进程有约一道题的抖动，见下文）：
...
按 65 题全集统计，数据来自 2026-09-13 的运行，早于次日两道数据库开发题的标注修正。所有策略跑同一批问题，因此用配对检验判断差异是否可信：
...
生成环节另有一层按采样的评测（`rag-qa evaluate-generation`）。同一个提示词在温度 0.7 下会得出
不同回答：2026-09-14 的上下文对照里，有一题两轮输入逐字相同，审查结论却翻转了，所以单次采样的
A/B 对照无法归因。这层评测冻结生成模型实际看到的上下文，每个方案对每题重复采样，再由另一家族的
评判模型把回答拆成事实陈述，逐条对照上下文核对。判为有依据的陈述必须附原文摘录，程序会在上下文里
找到这段摘录才算数。方案之间先按题取平均，再做配对的符号翻转检验。输出目录可以断点续跑，首次运行
即冻结实验协议，换了参数的重跑会被拒绝。

```bash
rag-qa evaluate-generation --cases artifacts/evaluations/generation-temperature-20260915/cases.json \
  --arm t07=baseline@0.7 --arm t02=baseline@0.2 --samples 5 \
  --output-dir artifacts/evaluations/generation-temperature-20260915/temperature
```

写成 `--arm 名称=上下文@stored` 时不调用生成模型，直接评判保存过的回答，可用来先检验评判器与既有审查是否一致。
首次校验（2026-09-15）没有通过预先登记的门槛，评判器结果暂不作为决策依据，见 [评判器校验记录](docs/generation-judge-validation-20260915.md)。
...
相关性按四级评分：标注标题精确匹配记3分，同深度且同父节的兄弟小节记2分，
有明确章号依据的同章内容记1分，其余记0分。局部列表编号不能推断章号，父节不算兄弟小节。
这些等级是标题结构的启发式代理，不代表片段已经包含答案。

nDCG按chunk粒度计算：每本书索引中的全部chunk按同一规则评分，排序后取Top-K作为理想收益。
重复chunk ID不重复计分。同一小节的不同chunk可以各自获得精确匹配分，分母也包含这些chunk。
没有候选全集的底层测试调用会标记 `returned_results_only`，只反映返回列表内部排序，
不能与正式 `corpus` 口径比较。旧版固定理想排名产生的nDCG应作废，不直接比较。

报告还统计**证据保留率**：把同一批检索结果按 `--context-budget`（默认 2000 字符）用问答实际的装入逻辑
打包，统计评为精确或兄弟小节的片段里有多少真正进了上下文，其中又有多少被截断。检索评测只看片段有没有
排进 Top-K，RAGAS 只看送进模型之后的上下文，「检索到了却装不下」这一段原本没有指标——两次握手题正是
丢在这里。这一步不调用任何模型，完全确定性。它是代理指标：片段在上下文里不等于答得出来，而且塞更多更短
的片段也能把它刷高，所以要和片段数、上下文字数一起看，并且只在相同预算下比较。`--context-budget 0`
可只评测检索。报告 schema 升级为 3。

65题、Top-5、本机MPS实测，同一套检索结果只改预算：
...
保留率是逐题平均，只在「Top-K里确实有相关片段」的题上有定义，四种策略的分母分别是51、57、58、62题，
所以跨策略不可直接比；「送达/取回」是全部65题的合计，才是可比的那一列。

默认预算下排序最好的策略送达的证据反而最少：Hybrid+Rerank 取回191条相关片段而 Embedding 取回180条，
装进上下文后却是前者154条、后者160条。Recall@5 数的是排进前五，2000字符平均只装得下4条，
排在第4、5位的相关片段会被丢掉；重排把更多相关片段挤进前五，也就有更多片段挤在预算边界之外。
预算提到4000后191条全部送达、截断从23条降到4条，而平均上下文字数只从1601涨到2336——多数题目的
Top-5本来就不到4000字符，2000正是实际卡住的那一刀。本轮只测量，未改默认值，也未验证送达更多片段
能否提高答案质量。

预算不参与检索，所以3000那一行的 Recall@5 0.908 不是预算造成的，而是下面那条重排跨进程抖动
（汉明码题）又出现了一次。

**含重排的结果跨进程不完全可复现。** 同一进程内重复检索逐位一致；五次独立进程运行中，BM25、Embedding、
Hybrid 三种策略逐题 Top-5 章节完全相同，只有 Hybrid + Reranker 在汉明码一题上换掉一个片段，使 65 题
Recall@5 在 0.877 与 0.892 之间变动（五次中两次为 0.877），幅度约一道题。此前实测被交换的两个候选
重排分差约 0.05，并非近似并列，所以早先归因于 MPS 浮点微差的说法不成立；具体原因尚未定位。
